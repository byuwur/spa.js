...
(function (global) {
   ...
   * /spa.php/ receives the active URI from PHP. Static /spa.js/ keeps the
   * browser routing mode in localStorage because there is no server rewrite.
   */
  bySPA.ROUTER_MODE = byStorage.getItem("ROUTER_MODE") ?? "hash";
  ...
  function fileHistory(url, replace = false) {
    const routeURL = bySPA.parseURL(url).url;
    const hash = `#${routeURL}`;
    try {
  ...
  function historyPush(url) {
    ...
    if (window.location.protocol === "file:") return fileHistory(url);
    history.pushState({ index: bySPA.HISTORY_INDEX, url }, "", browserURL(url));
  }
  ...
  function historyReplace(url) {
    ...
    bySPA.HISTORY_PATH[bySPA.HISTORY_INDEX] = url;
    // === /spa.js/ only: hash/file history instead of the PHP path URL ===
    if (window.location.protocol === "file:") return fileHistory(url, true);
    history.replaceState({ index: bySPA.HISTORY_INDEX, url }, "", browserURL(url));
  }
  ...
  bySPA.errorPage = function (status, custom_error_message = "", navigationId = bySPA.NAVIGATION_ID) {
    ...
    const requestError = function (path) {
      return $.ajax({
        ...
        .catch(function () {
          if (navigationId !== bySPA.NAVIGATION_ID) return null;
          // === /spa.js/ only: A missing candidate is not the error page; try the next one ===
    ...
    const loadError = function (paths, index = 0) {
      return requestError(`${paths[index]}`).then(function (data) {
        if (navigationId !== bySPA.NAVIGATION_ID) return null;
        if (data || index + 1 >= paths.length) return data;
  ...
  bySPA.parseURL = function (uri = "/") {
    ...
    if (uri.includes("://")) {
      try {
        ...
        // === /spa.js/ only: accept hash routes in full browser URLs ===
        uri = hashToURL(parsed.hash) ?? parsed.pathname + parsed.search;
      } catch (e) {
        uri = "/";
      }
    } else {
  ...
  function routeURL(uri = "/") {
    ...
    const get = { ...query, ...params, ...queryFromURL(route?.URI), ...(route?.GET ?? {}) };
    ...
    if (uri == "") {
      // === /spa.js/ only: keep empty URI routes from losing the current page ===
   ...
   * Each call owns a new navigation generation; older page, component, error,
   * loader, language, and lifecycle results cannot mutate the current route.
   * Emits bySPA:before-unload, followed by bySPA:load or bySPA:error.
   * @param {object} mode History handling options (`push` and `replace`).
  ...
  bySPA.load = function (url, mode = { push: true }) {
    ...
    return $.ajax({
      ...
      .then(function (data) {
        // Correctness does not depend on aborting XHR: stale responses simply
        // lose ownership of all DOM and lifecycle side effects.
        if (navigationId !== bySPA.NAVIGATION_ID) return null;
  ...
  function init() {
    ...
    window.addEventListener("hashchange", function () {
      ...
      const nextURL = hashToURL(window.location.hash);
